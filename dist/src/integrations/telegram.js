...
function getUpdates() {
    return __awaiter(this, arguments, void 0, function* (offset = 0) {
        try {
            const res = yield (0, node_fetch_1.default)(`${TELEGRAM_API_URL}/getUpdates?timeout=30&offset=${offset}`);
            const data = yield res.json();
            if (!data.ok) {
                logger_1.default.error('Telegram getUpdates API error: %s', data.description);
                return [];
            }
            return data.result;
        }
        catch (err) {
            logger_1.default.error('Failed to fetch updates from Telegram: %o', err);
            return [];
        }
    });
}
